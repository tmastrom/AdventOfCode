...
export async function partOne() {
    ...
    for await (const line of file.readLines()) {
        ...
        if (Math.max(...currentGame['red']) <= maximums['red'] && Math.max(...currentGame['green']) <= maximums['green'] && Math.max(...currentGame['blue']) <= maximums['blue']) {
...
export async function partTwo() {
    ...
    for await (const line of file.readLines()) {
        ...
        const power = Math.max(...currentGame['red']) * Math.max(...currentGame['green']) * Math.max(...currentGame['blue'])
        ans += power
    }
